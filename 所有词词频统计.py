...
stopwords = set([
    ...
    "发现", "每天", "不是", "这样", "或者", "怎么", "开始", "不好", "时候","可以","自己","所以","如何","www","question","https","com","zhihu","怎样","图片","达到","经常","最好","现在","为什么"
])
...
def analyze_words(data):
    word_count = {}
    for row in data:
        combined_text = ' '.join(filter(None, row))  # 合并行中的所有列，过滤掉空值
...
def write_to_database(word_count):
    ...
    try:
        with conn.cursor() as cursor:
            # 创建表格，如果不存在的话
            create_table_query = """
            CREATE TABLE IF NOT EXISTS cipin1 (
            ...
            # 清空表格，准备写入新数据
            clear_table_query = "TRUNCATE TABLE cipin1"
            cursor.execute(clear_table_query)
            
            # 插入数据
            insert_query = "INSERT INTO cipin1 (word, count) VALUES (%s, %s)"
